...
const mkLineComment = (value: string) => (start: number): LineComment => ({type: "LineComment", value, range: [start, start + value.length + 2]})
const mkBlockComment = (value: string) => (start: number): BlockComment => ({type: "BlockComment", value, range: [start, start + value.length + 4]})
...
const singleQuotedString: P.Parser<string, String> =
  P.surroundedBy
    (C.char("'"))
...
const lineUntilEnd = P.manyTill(
  P.sat((s: string) => s !== "\n"),
  P.either<string, string | void>(S.string("\n"), P.eof)
)

const lineComment = pipe(
  P.withStart(P.seq(lineCommentStart, () => lineUntilEnd)),
  P.map(([chars, stream]) => {
    const value = chars.join('')

    return mkLineComment(value)(stream.cursor)
  }),
)

const blockComment = pipe(
  P.withStart(P.seq(blockCommentStart, () => P.manyTill(anyChar, blockCommentEnd))),
  P.map(([chars, stream]) => {
    const value = chars.join('')

    return mkBlockComment(value)(stream.cursor)
  }),
)
...
export const extractComments = (input: string): Comment[] => 
  pipe(
    run(allComments, input),
    E.fold(
      (error) => {
...
console.log(extractComments(input))
